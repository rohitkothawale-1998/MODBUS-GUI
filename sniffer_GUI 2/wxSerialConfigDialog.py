...
class SerialConfigDialog(wx.Dialog):
        ...
        try:
            if sys.platform.startswith('win'):
                try:
                    ...
                    # Fallback to COM ports if serialutils is not available
                    ports = ['COM' + str(i + 1) for i in range(256)]
            elif sys.platform.startswith('linux') or sys.platform.startswith('cygwin'):
                ports = glob.glob('/dev/tty[A-Za-z]*')
            elif sys.platform.startswith('darwin'): #MacOS X
                # Include both tty and cu devices
                ports = glob.glob('/dev/tty.*') + glob.glob('/dev/cu.*')
                # Filter out some common non-serial devices
                ports = [p for p in ports if not any(x in p for x in ['Bluetooth', 'Bluetooth-Incoming-Port'])]
            else:
            ...
            for portname in ports:
                ...
                if self.serial.portstr == portname:
                    index = self.combo_box_port.GetCount() - 1

            if self.serial.portstr is not None:
                self.combo_box_port.SetValue(str(self.serial.portstr))
            elif self.combo_box_port.GetCount() > 0:
                self.combo_box_port.SetSelection(0)
        except Exception as e:
            print(f"Error enumerating ports: {e}")
            # Add at least one dummy port to prevent UI issues
            self.combo_box_port.Append("No ports found")
            self.combo_box_port.SetSelection(0)
        if self.show & SHOW_BAUDRATE:
    ...
    def __do_layout(self):
        ...
        if self.show & SHOW_BAUDRATE:
            ...
            sizer_baudrate.Add(self.choice_baudrate, 1, wx.ALIGN_CENTER_VERTICAL, 0)
